...
# - Applies NVIDIA + Hyprland tuning (mkinitcpio/modprobe/grub)
...
configure_nvidia() {
  log "Configuring NVIDIA for Hypland (modprobe + mkinitcpio + grub)"
...
configure_hiddify_if_present() {
  if [[ -f /usr/lib/hiddify/HiddifyCli ]]; then
    ...
    sudo setcap 'cap_net_admin,cap_net_bind_service,cap_net_raw=+eip' /usr/lib/hiddify/hiddify || true
  fi
}
...
1) Log out and choose Hyprland in your display manager.
